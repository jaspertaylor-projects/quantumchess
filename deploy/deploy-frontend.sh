...
# Every HTML page, including nested /guides/*.html articles, must revalidate.
# Otherwise a corrected guide could remain cached for a full year.
aws s3 sync dist "s3://$BUCKET" \
  --exclude "*" \
  --include "*.html" \
  --cache-control "no-cache"
# Other root files that change but aren't content-hashed must also revalidate.
for f in ads.txt humans.txt robots.txt sitemap.xml; do
