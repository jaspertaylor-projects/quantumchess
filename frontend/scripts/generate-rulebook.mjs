...
import { RULE_GROUPS, TUTORIAL_COPY, TURN_INTRO, TURN_STEPS } from '../src/rules/rulebookPresentation.js';
...
<section class="qr-turn-order" aria-labelledby="qr-turn-title"><span class="qr-eyebrow">Start here</span><h2 id="qr-turn-title">Turn sequence</h2><p>${escape(TURN_INTRO)}</p><ol>${TURN_STEPS.map((step) => `<li><strong>${escape(step.title)}</strong><span>${escape(step.text)}</span></li>`).join('')}</ol></section>
