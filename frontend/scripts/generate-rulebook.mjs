...
import { RULE_GROUPS, TUTORIAL_COPY } from '../src/rules/rulebookPresentation.js';
...
const state = (rows) => rows.map((row) => `<div class="qr-static-state"><span class="qr-state-label">${escape(row.label)}</span><div class="qr-static-identities">${[...row.types].map((type) => `<span>${types[type]}</span>`).join('')}</div></div>`).join('');
const illustration = (rule) => rule.before ? `<figure class="qr-experiment qr-experiment--${rule.id}"><div class="qr-experiment-label">${escape(rule.action)}</div><div class="qr-static-comparison"><div><span class="qr-state-label">Before</span>${state(rule.before)}</div><div><span class="qr-state-label">After</span>${state(rule.after)}</div></div><figcaption>${escape(rule.caption)}</figcaption></figure>` : '';
const reference = (rule) => `<details class="qr-details" id="qr-rule-${rule.id}"><summary>${escape(rule.title)}<span aria-hidden="true">+</span></summary><div class="qr-details-content"><h3>${escape(rule.concept)}</h3><p>${escape(rule.summary)}</p>${rule.paragraphs.map((p) => `<p>${escape(p)}</p>`).join('')}${illustration(rule)}<p>${escape(rule.detail)}</p></div></details>`;
const html = `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#070915"><title>The Rulebook — Quantum Chess</title><meta name="description" content="Learn Quantum Chess on the board with the guided tutorial, or explore the rules of superposition, measurement, contact and conservation."><link rel="icon" type="image/x-icon" href="/favicon.ico"><link rel="canonical" href="https://quantumchess.ninja/rules.html"><style>${css}</style></head>
...
<footer class="qr-legal"><a href="/privacy.html">Privacy</a><a href="/terms.html">Terms</a></footer></main></body></html>\n`;
