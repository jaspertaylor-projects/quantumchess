...
export default function UnstableLinePanel({
  ...
  if (!page) {
    const lockedText = !effectiveUser
      ...
      : unlocked
        ? `Route map ready: ${mapBots.length} bots plus Rudolf Einstein.`
    ...
    <section style={{ display: 'grid', gap: page ? 18 : 12 }}>
      ...
      >
        ...
        <div style={{ fontSize: 12.5, color: theme.textSecondary, lineHeight: 1.4 }}>
          A double-slit map with {mapBots.length} target bots plus Rudolf Einstein. Bot nodes add lasting downsides; pickup spaces add lasting upsides.
        </div>
      ...
      {displayRun ? (
        ...
        <button type="button" onClick={ensureRun} disabled={busy} style={{ ...baseButton, cursor: busy ? 'default' : 'pointer' }}>
          ...
          <span style={{ fontSize: 12.5, color: theme.textSecondary }}>Place every eligible bot and chart a route to Rudolf Einstein.</span>
        </button>
      )}
    </section>
  );
